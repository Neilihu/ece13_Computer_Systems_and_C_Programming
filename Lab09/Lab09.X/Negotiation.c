...
{
   // NegotiationData square;// = secret^2;
    NegotiationData result  = (secret * secret) % PUBLIC_KEY;
    return result;
}
...
{
    //NegotiationData 
    uint16_t a = (secret * secret) % PUBLIC_KEY;
    if (a == commitment) {
        return TRUE;
    } else {
        return FALSE;
...
NegotiationOutcome NegotiateCoinFlip(NegotiationData A, NegotiationData B)
{
    ...
    while (bitstring < 8) {
        if ((A_xor_B & 0x01) == 1) { // AND GATE Checking if the first bit is zero
            onesCounter = onesCounter + 1;
        }
        bitstring = bitstring + 1;
        A_xor_B = A_xor_B >> 1; // Getting rid of the bit we just read
    }
